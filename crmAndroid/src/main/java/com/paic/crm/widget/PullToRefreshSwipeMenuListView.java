...
public class PullToRefreshSwipeMenuListView extends ListView implements OnScrollListener {
	...
	private SwipeMenuInteractAdapter mMenuInteract;
	...
	public void setAdapter(ListAdapter adapter) {
		...
		super.setAdapter(new SwipeMenuAdapter(getContext(), adapter) {
			...
			@Override
			protected void interactWithUser(int position, SwipeMenu menu) {
				if(mMenuInteract!=null){
					mMenuInteract.interactWithUser(position,menu);
				}
				super.interactWithUser(position, menu);
			}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if ((mTouchView == null || !mTouchView.isActivated()) && Math.pow(dx, 2) / Math.pow(dy, 2) <= 3) {
...
//		int height = mFooterView.getBottomMargin() + (int) delta;
	...
	public void setMenuInteract(SwipeMenuInteractAdapter menuInteract) {
		this.mMenuInteract = menuInteract;
	}
}
