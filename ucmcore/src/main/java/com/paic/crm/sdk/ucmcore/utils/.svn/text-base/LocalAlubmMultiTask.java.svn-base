 ...
 *
 * 用于替换系统自带的AsynTask，
 * 使用自己的多线程池，执行一些比较复杂的工作，
 * 比如select photos，
 * 这里用的是缓存线程池，
 * 也可以用和cpu数相等的定长线程池以提高性能
 */
public abstract class LocalAlubmMultiTask<Params, Progress, Result> extends AsyncTask<Params, Progress, Result> {
    ...
    public void executeDependSDK(Params... params) {
        if (mTHREAD_POOL_EXECUTOR == null) {
            synchronized (LocalAlubmMultiTask.class) {
                if (mTHREAD_POOL_EXECUTOR == null) {
                    initThreadPool();
                }
    ...
    private void initThreadPool() {
        mTHREAD_POOL_EXECUTOR = new ThreadPoolExecutor(CORE_POOL_SIZE
                , MAXIMUM_POOL_SIZE
                , KEEP_ALIVE
                , TimeUnit.SECONDS
                , sPoolWorkQueue
